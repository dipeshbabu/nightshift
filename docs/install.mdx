...
`nightshift install` creates a fully isolated agent environment at a given prefix directory. It downloads all required tools, scaffolds a Python workspace, installs dependencies, and initializes a git repository, this is everything the agent needs to start working.
...
After tools are installed, Nightshift creates a [uv workspace](https://docs.astral.sh/uv/concepts/projects/workspaces) with:
...
This ensures agents operate in a reproducible environment regardless of what's installed on the host system. This also keeps the agent off the host system.
